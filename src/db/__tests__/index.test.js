...
class MockTable {
  ...
  where(field) {
    ...
    return {
      equals: (val) => ({
        ...
        delete: async () => {
          for (const [k, v] of d) if (v[field] === val) d.delete(k)
        },
      ...
      anyOf: (vals) => ({
        ...
        delete: async () => {
          for (const [k, v] of d) if (vals.includes(v[field])) d.delete(k)
        },
        primaryKeys: async () =>
          [...d.values()].filter((r) => vals.includes(r[field])).map((r) => r.id),
      }),
    }
  }
  ...
  filter(fn) {
    ...
    return {
      delete: async () => {
        for (const [k, v] of d) if (fn(v)) d.delete(k)
      },
      modify: async (ch) => {
        for (const [k, v] of d) if (fn(v)) d.set(k, { ...v, ...ch })
      },
...
describe('getCharacterStateMap', () => {
  ...
  it('returns Map with latest state per character name', async () => {
    await addStoryFact({
      ...
      content: 'Alice: at the village, happy',
      priority: 'high',
    })
    await new Promise((r) => setTimeout(r, 2))
    await addStoryFact({
      storyId: 'story',
      factType: 'character_state',
      content: 'Alice: at the castle, fearful',
      priority: 'high',
    })
