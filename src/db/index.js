...
export async function deleteStory(id) {
  ...
  if (db.character_relationships)
    await db.character_relationships.where('storyId').equals(id).delete()
...
export async function addScene(scene) {
  const id = crypto.randomUUID()
  const createdAt = Date.now()
  const order = scene.order ?? (await db.scenes.where('chapterId').equals(scene.chapterId).count())
...
export async function replaceStoryFactsForScenes(storyId, sceneIds, newFacts) {
  ...
  const toDelete = existing
    .filter((f) => f.sourceSceneId && sceneIds.includes(f.sourceSceneId))
    .map((f) => f.id)
...
async function insertExampleStory(locale) {
  ...
  await db.transaction(
    'rw',
    [db.stories, db.characters, db.chapters, db.scenes, db.ideas],
    async () => {
...
export async function exportProject() {
  try {
    ...
    try {
      if (db.character_relationships)
        characterRelationships = await db.character_relationships.toArray()
    } catch (_) {}
    ...
    throw createStorageError('Could not create backup. Try again or copy your text elsewhere.', e)
  }
}
...
export function validateImportData(data) {
  ...
  if (data.characterRelationships != null && !Array.isArray(data.characterRelationships)) {
    throw createImportValidationError(
      'Invalid backup format: characterRelationships must be an array.'
    )
  }
}
    ...
    await db.transaction('rw', stores, async () => {
      ...
      if (isV2 && data.stories?.length) {
        ...
        for (const i of data.ideas || [])
          await db.ideas.add({ ...i, storyId: i.storyId ?? storyId })
        for (const c of data.characters || [])
          await db.characters.add({ ...c, storyId: c.storyId ?? storyId })
        for (const ch of data.chapters || [])
          await db.chapters.add({ ...ch, storyId: ch.storyId ?? storyId })
        for (const s of data.scenes || []) await db.scenes.add(s)
        if (db.idea_custom_types && Array.isArray(data.ideaCustomTypes)) {
          for (const ct of data.ideaCustomTypes) await db.idea_custom_types.add(ct)
        }
