<template>
  <div class="card form-card idea-form-card">
    ...
    <div class="form-actions">
      ...
      <button v-if="idea" type="button" class="btn btn-ghost" @click="doDelete">{{ t('ideas.delete') }}</button>
      <button type="button" class="btn btn-primary" @click="doSave">{{ idea ? t('ideas.save') : t('ideas.add') }}</button>
    </div>
...
watch(
  ...
  () => {
    if (props.idea) {
      form.value = { title: props.idea.title || '', body: props.idea.body || '', type: props.idea.type || 'plot' };
    } else {
