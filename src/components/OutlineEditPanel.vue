      ...
      >
        <div class="ep-panel" role="dialog" :aria-modal="true" :aria-label="panelTitle">
          <div class="ep-header">
            ...
            <button
              type="button"
              class="ep-close btn btn-ghost btn-icon"
              :aria-label="t('ideas.cancel')"
              @click="$emit('close')"
            >
              <svg
                width="16"
          ...
          <div class="ep-body">
            ...
            <template v-if="type === 'chapter'">
              <div class="ep-form-group">
                ...
                <div class="ep-beat-grid">
                  ...
                  >
                    <span
                      class="ep-beat-dot"
                      :style="b ? `background:${beatColor(b)}` : 'background:var(--text-muted)'"
                    ></span>
                    {{ beatLabel(b) }}
                  </button>
                </div>
              ...
              <div class="ep-form-group">
                ...
                />
                <AiExpandButton
                  :current-value="form.summary"
                  :field-name="t('outline.chapterSummary')"
                  :extra-context="extraContext"
                  @expanded="form.summary = $event"
                />
              </div>
            </template>
            ...
            <template v-else-if="type === 'scene'">
              ...
              <div class="ep-form-group">
                ...
                <input
                  ...
                  v-model="form.title"
                  type="text"
                  :placeholder="t('outline.sceneTitlePlaceholder')"
                />
              </div>
              <div class="ep-form-group">
                <label class="ep-label">{{ t('outline.oneSentenceSummary') }}</label>
                <ResizableTextarea
                  v-model="form.oneSentenceSummary"
                ...
                />
                <AiExpandButton
                  :current-value="form.oneSentenceSummary"
                  :field-name="t('outline.sceneOneSentence')"
                  :extra-context="extraContext"
                  @expanded="form.oneSentenceSummary = $event"
                />
              </div>
              <div class="ep-form-group">
          ...
          <div class="ep-footer">
            ...
            <button type="button" class="btn btn-primary" @click="onSave">
              {{ isNew ? t('ideas.add') : t('ideas.save') }}
            </button>
          </div>
        </div>
      </div>
...
.ep-enter-active .ep-panel,
.ep-leave-active .ep-panel {
...
.ep-enter-from .ep-panel {
  transform: translateX(100%);
}
.ep-leave-to .ep-panel {
  transform: translateX(100%);
}
@media (max-width: 767px) {
  .ep-enter-from .ep-panel {
