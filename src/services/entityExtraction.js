...
export async function extractNewEntities({ sceneText, storyId }) {
  ...
  const userPrompt = EXTRACT_USER
    .replace("{{KNOWN}}", knownBlock.slice(0, 2000))
    .replace("{{TEXT}}", sceneText.slice(0, 6000));
  ...
  try {
    ...
    const VALID_TYPES = new Set([
      "character", "location", "world", "magic_system", "symbol", "mystery", "other",
    ]);
