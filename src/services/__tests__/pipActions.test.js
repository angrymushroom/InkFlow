...
describe('applySingleAction', () => {
  ...
  describe('add_chapter', () => {
    it('calls addChapter with correct title and storyId', async () => {
      ...
      expect(mockAddChapter).toHaveBeenCalledWith(expect.objectContaining({ title: 'Act One', storyId: 'story1' }));
    });
    ...
    it('appends to end when after_chapter_title not given', async () => {
      ...
      await applySingleAction({ raw: { type: 'add_chapter', title: 'New Chapter' }, storyId: 'story1' });
  ...
  describe('update_chapter', () => {
    it('calls updateChapter when title_match finds a chapter', async () => {
      ...
      expect(mockUpdateChapter).toHaveBeenCalledWith('ch1', expect.objectContaining({ title: 'Act 1' }));
    });

    it('throws when no chapter matches title_match', async () => {
      mockGetChapters.mockResolvedValue([]);
      await expect(
        applySingleAction({ raw: { type: 'update_chapter', title_match: 'Missing', fields: { title: 'X' } }, storyId: 'story1' })
      ).rejects.toThrow();
    });
  });
  ...
  describe('add_scene', () => {
    it('calls addScene with correct title and chapterId', async () => {
      ...
      expect(mockAddScene).toHaveBeenCalledWith(expect.objectContaining({ title: 'The Opening', chapterId: 'ch1' }));
    });

    it('throws when chapter_title_match finds no chapter', async () => {
      mockGetChapters.mockResolvedValue([]);
      await expect(
        applySingleAction({ raw: { type: 'add_scene', title: 'X', chapter_title_match: 'Missing' }, storyId: 'story1' })
      ).rejects.toThrow();
    });
  });
  ...
  describe('update_scene', () => {
    it('calls updateScene when title_match finds a scene', async () => {
      ...
      await applySingleAction({
        raw: { type: 'update_scene', title_match: 'The Opening', fields: { title: 'The New Opening' } },
        storyId: 'story1',
      });
      expect(mockUpdateScene).toHaveBeenCalledWith('s1', expect.objectContaining({ title: 'The New Opening' }));
    });

    it('throws when no scene matches title_match', async () => {
      mockGetScenes.mockResolvedValue([]);
      await expect(
        applySingleAction({ raw: { type: 'update_scene', title_match: 'Missing', fields: { title: 'X' } }, storyId: 'story1' })
      ).rejects.toThrow();
    });
  });
  ...
  describe('upsert_character', () => {
    it('calls addCharacter when name does not exist (case-insensitive)', async () => {
      ...
      expect(mockAddCharacter).toHaveBeenCalledWith(expect.objectContaining({ name: 'Alice', storyId: 'story1' }));
    ...
    it('calls updateCharacter when character already exists', async () => {
      ...
      expect(mockUpdateCharacter).toHaveBeenCalledWith('c1', expect.objectContaining({ name: 'ALICE' }));
  ...
  describe('update_spine', () => {
    it('calls saveStory with updated spine fields', async () => {
      const mockStory = { id: 'story1', oneSentence: 'Old sentence', template: 'snowflake', templateFields: {} };
    ...
    it('dispatches inkflow-story-saved event', async () => {
      mockGetStoryById.mockResolvedValue({ id: 'story1', template: 'snowflake', templateFields: {} });
      const events = [];
      const handler = (e) => events.push(e.type);
      window.addEventListener('inkflow-story-saved', handler);
      ...
      window.removeEventListener('inkflow-story-saved', handler);
      expect(events).toContain('inkflow-story-saved');
    });

    it('preserves existing fields not mentioned in action', async () => {
      const mockStory = {
        id: 'story1', oneSentence: 'Old', setup: 'Existing setup',
        template: 'snowflake', templateFields: {},
      };
      mockGetStoryById.mockResolvedValue(mockStory);
      let savedStory = null;
      mockSaveStory.mockImplementation((s) => { savedStory = s; return Promise.resolve(); });
