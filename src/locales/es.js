export default {
  common: {
    unsavedLeave: 'Tienes cambios sin guardar. ¿Salir de todos modos?',
    loadErrorGeneric: 'No se pudo cargar. Vuelve a intentarlo o revisa el respaldo en Ajustes.',
    saveErrorGeneric:
      'No se pudo guardar. En navegación privada o sin espacio, descarga un respaldo en Ajustes.',
  },
  nav: {
    ideas: 'Ideas',
    entities: 'Entidades',
    story: 'Historia',
    characters: 'Personajes',
    outline: 'Esquema',
    write: 'Escribir',
    settings: 'Ajustes',
    chapters: 'Capítulos',
    noChapters: 'Aún no hay capítulos',
    goToOutline: 'Ir al esquema',
  },
  sidebar: {
    contents: 'Contenido',
    story: 'Historia',
    ideas: 'Ideas',
    characters: 'Personajes',
    outline: 'Esquema',
    chapterNum: 'Capítulo {n}',
    sceneNum: 'Escena {n}',
    noIdeas: 'Aún no hay ideas.',
    noCharacters: 'Aún no hay personajes.',
    entities: 'Entidades',
    noEntities: 'Aún no hay entidades.',
    addInTab: 'Añadir en la pestaña',
    untitledStory: 'Historia sin título',
    editCurrentStory: 'Editar historia actual',
    noStories: 'Aún no hay historias.',
    goToStory: 'Ir a Historia',
    newStory: 'Nueva historia',
  },
  ideas: {
    title: 'Ideas',
    subtitle:
      'Captura tramas, rasgos de personaje, mundo o escenas. Sin estructura todavía — solo captura.',
    newIdea: 'Nueva idea',
    editIdea: 'Editar idea',
    titleLabel: 'Título',
    bodyLabel: 'Cuerpo',
    typeLabel: 'Tipo',
    shortTitle: 'Título corto',
    bodyPlaceholder:
      'Idea rápida... Escribe /ai y haz clic en Expandir con IA para desarrollarla con tu Historia e Ideas.',
    plot: 'Trama',
    subplot: 'Subtrama',
    scene: 'Escena',
    event: 'Evento',
    character: 'Personaje',
    relationship: 'Relación',
    faction: 'Facción / grupo',
    world: 'Mundo / ambientación',
    location: 'Lugar / sitio',
    culture: 'Cultura / sociedad',
    item: 'Objeto / artefacto',
    creature: 'Criatura / especie',
    magic_system: 'Sistema mágico',
    technology: 'Tecnología / ciencia',
    concept: 'Concepto / tema',
    conflict: 'Conflicto',
    mystery: 'Misterio / pista',
    symbol: 'Símbolo / motivo',
    prophecy: 'Profecía / anticipación',
    other: 'Otro',
    addCustomType: 'Añadir tipo personalizado…',
    typeGroupSuggested: 'Sugeridos',
    typeGroupCustom: 'Personalizados',
    cancel: 'Cancelar',
    add: 'Añadir',
    save: 'Guardar',
    untitled: 'Sin título',
    edit: 'Editar',
    delete: 'Eliminar',
    empty: 'Aún no hay ideas. Añade la primera.',
    ideaBody: 'Cuerpo de la idea',
    confirmDelete: '¿Eliminar esta idea?',
    customTypeModalTitle: 'Nuevo tipo de idea',
    customTypeModalPlaceholder: 'Nombre del tipo',
    customTypeModalSave: 'Añadir tipo',
    customTypeModalNameRequired: 'Escribe un nombre.',
    manageCustomTypes: 'Gestionar tipos',
    manageCustomTypesTitle: 'Tipos de idea personalizados',
    customTypeRename: 'Renombrar',
    customTypeDelete: 'Eliminar',
    noCustomTypes: 'Sin tipos personalizados.',
    close: 'Listo',
  },
  entities: {
    title: 'Entidades',
    subtitle:
      'Gestiona personajes, lugares, objetos y demás elementos de la historia. Elige un tipo para ver y editar.',
    filterByType: 'Filtrar por tipo',
    typeCharacters: 'Personajes',
    addCharacter: 'Añadir personaje',
    addEntity: 'Añadir {type}',
    emptyFilter: 'No hay entidades de este tipo.',
    selectOrAdd: 'Selecciona una entidad para editar o añade una nueva.',
    noEntitiesYet: 'Aún no hay entidades. Crea la primera.',
    noTypeYet: 'Aún no hay {type}. Añade una.',
    addAnotherType: 'Añadir otro tipo…',
  },
  templates: {
    snowflake: {
      name: 'Copo de nieve',
      subtitle:
        'Estructura descendente: una frase → planteamiento → tres desastres escalantes → final.',
      fields: {
        oneSentence: 'Resumen en una frase',
        setup: 'Planteamiento',
        disaster1: 'Desastre 1',
        disaster2: 'Desastre 2',
        disaster3: 'Desastre 3',
        ending: 'Final',
      },
      placeholders: {
        oneSentence: 'Tu historia en una frase (quién, qué, apuestas).',
        setup: 'Situación inicial y mundo.',
        disaster1: 'Primer gran punto de inflexión.',
        disaster2: 'Segundo gran punto de inflexión.',
        disaster3: 'Tercer gran punto de inflexión / clímax.',
        ending: 'Resolución.',
      },
    },
    save_the_cat: {
      name: 'Salva al gato',
      subtitle:
        'Estructura en beats: logline → catalizador → diversión → punto medio → todo perdido → final.',
      fields: {
        logline: 'Logline',
        catalyst: 'Catalizador (Acto 1)',
        debate: 'Debate y entrada al Acto 2',
        midpoint: 'Punto medio',
        allIsLost: 'Todo está perdido',
        finale: 'Final (Acto 3)',
      },
      placeholders: {
        logline: 'Premisa en una frase: personaje, conflicto, apuestas.',
        catalyst: 'El incidente desencadenante que rompe el mundo ordinario (aprox. 10%).',
        debate: 'El protagonista duda en actuar, luego se compromete (aprox. 15%).',
        midpoint: 'Falsa victoria o falsa derrota que sube las apuestas (aprox. 50%).',
        allIsLost: 'El momento más oscuro — todo parece perdido (aprox. 75%).',
        finale: 'Cómo el protagonista cambia y resuelve el conflicto (aprox. 85-99%).',
      },
    },
    story_circle: {
      name: 'Círculo narrativo',
      subtitle:
        'Círculo de Dan Harmon en 8 segmentos: personaje en equilibrio → necesidad → viaje → transformación.',
      fields: {
        you: 'Tú (equilibrio)',
        need: 'Necesidad',
        go: 'Partir',
        search: 'Buscar',
        find: 'Encontrar',
        take: 'Pagar el precio',
        return: 'Retorno',
        change: 'Cambio',
      },
      placeholders: {
        you: '¿Quién es el protagonista? Su mundo ordinario y cómodo.',
        need: 'Lo que necesita — aunque todavía no lo sepa.',
        go: 'Entra en una situación o mundo desconocido.',
        search: 'Se adapta, intenta, conoce aliados y enemigos.',
        find: 'Encuentra lo que buscaba.',
        take: 'Pero paga un alto precio por ello.',
        return: 'Vuelve a su mundo familiar.',
        change: 'El viaje lo ha transformado.',
      },
    },
    hero_journey: {
      name: 'El viaje del héroe',
      subtitle:
        'Monomito de Campbell: el héroe abandona su mundo, afronta pruebas, supera la ordalia, regresa transformado.',
      fields: {
        premise: 'Premisa',
        call: 'La llamada a la aventura',
        trials: 'El camino de las pruebas',
        ordeal: 'La ordalia suprema',
        elixir: 'El regreso con el elixir',
      },
      placeholders: {
        premise: 'El héroe, su mundo ordinario y lo que está en juego.',
        call: 'El evento o persona que aleja al héroe de su mundo familiar.',
        trials: 'Las pruebas, aliados y enemigos encontrados en el camino.',
        ordeal: 'El momento más oscuro — una muerte y renacimiento simbólicos.',
        elixir: 'Lo que el héroe trae de vuelta y cómo ha sido transformado.',
      },
    },
    kishotenketsu: {
      name: '起承转合',
      subtitle:
        'Estructura este-asiática en 4 actos: introducir (起), desarrollar (承), giro (转, sin conflicto necesario), resolver (合).',
      fields: {
        ki: '起 — Introducción',
        sho: '承 — Desarrollo',
        ten: '转 — Giro',
        ketsu: '合 — Resolución',
      },
      placeholders: {
        ki: 'Introduce los personajes, el escenario y la situación inicial.',
        sho: 'Desarrolla y profundiza — enriquece el mundo y los personajes.',
        ten: 'Un giro inesperado, nueva perspectiva o yuxtaposición. No es un conflicto — es un desplazamiento.',
        ketsu: 'Reúne todos los hilos y encuentra una resolución.',
      },
    },
  },
  story: {
    title: 'Historia',
    subtitle:
      'Pasos 1–2 del copo de nieve: una frase, luego desarrollo (planteamiento, tres desastres y final).',
    templateLabel: 'Plantilla',
    oneSentence: 'Resumen en una frase',
    oneSentencePlaceholder:
      'Tu historia en una frase (quién, qué, qué está en juego). Usa /ai para enriquecer desde Ideas.',
    setup: 'Planteamiento',
    setupPlaceholder:
      'Situación inicial y mundo. Usa /ai + Expandir con IA para relacionar tus Ideas.',
    disaster1: 'Desastre 1',
    disaster1Placeholder: 'Primer giro importante. Escribe una idea corta, /ai y Expandir con IA.',
    disaster2: 'Desastre 2',
    disaster2Placeholder: 'Segundo giro',
    disaster3: 'Desastre 3',
    disaster3Placeholder: 'Tercer giro / clímax',
    ending: 'Final',
    endingPlaceholder: 'Resolución',
    saveStory: 'Guardar esqueleto de historia',
    saved: 'Guardado',
    saveError:
      'No se pudo guardar. Si estás en navegación privada o sin espacio, descarga un respaldo y usa una ventana normal.',
    fieldOneSentence: 'Resumen en una frase',
    fieldSetup: 'Planteamiento',
    fieldDisaster1: 'Desastre 1',
    fieldDisaster2: 'Desastre 2',
    fieldDisaster3: 'Desastre 3',
    fieldEnding: 'Final',
    deleteStorySection: 'Eliminar historia',
    deleteStory: 'Eliminar esta historia',
    deleteStoryConfirmTitle: '¿Eliminar esta historia?',
    deleteStoryConfirmBody:
      'Se eliminará permanentemente esta historia y todas sus ideas, personajes, capítulos y escenas. Esta acción no se puede deshacer.',
    deleteStoryConfirmButton: 'Eliminar historia',
    cannotDeleteLastStory: 'Debes tener al menos una historia.',
    progress: {
      spine: 'Estructura narrativa',
      characters: 'Personajes',
      outline: 'Esquema',
      writing: 'Escritura',
      chapters: 'capítulos',
      scenes: 'escenas',
    },
  },
  characters: {
    title: 'Personajes',
    subtitle:
      'Paso 3: para cada personaje principal — nombre, una frase, objetivo, motivación, conflicto, epifanía.',
    newCharacter: 'Nuevo personaje',
    editCharacter: 'Editar personaje',
    name: 'Nombre',
    namePlaceholder: 'Nombre del personaje',
    oneSentence: 'Una frase',
    oneSentencePlaceholder: 'Este personaje en una frase. Usa /ai + Expandir con IA.',
    goal: 'Objetivo',
    goalPlaceholder: 'Qué quiere',
    motivation: 'Motivación',
    motivationPlaceholder: 'Por qué lo quiere',
    conflict: 'Conflicto',
    conflictPlaceholder: 'Qué se interpone',
    epiphany: 'Epifanía',
    epiphanyPlaceholder: 'Qué aprende o descubre',
    empty: 'Aún no hay personajes. Añade el primero.',
    addFirst: 'Añadir tu primer personaje',
    unnamed: 'Sin nombre',
    fieldOneSentence: 'Una frase del personaje',
    fieldGoal: 'Objetivo del personaje',
    fieldMotivation: 'Motivación del personaje',
    fieldConflict: 'Conflicto del personaje',
    fieldEpiphany: 'Epifanía del personaje',
    confirmDelete: '¿Eliminar este personaje?',
    relationships: 'Relaciones',
    relationshipsSubtitle:
      'Define conexiones entre personajes: rivalidades, alianzas, familia y más.',
    newRelationship: 'Nueva relación',
    relFrom: 'Personaje A',
    relTo: 'Personaje B',
    relLabel: 'Tipo de relación',
    relLabelPlaceholder: 'ej. rivales, aliados, hermanos…',
    relDescription: 'Descripción (opcional)',
    relDescriptionPlaceholder: 'Notas sobre esta relación',
    relConfirmDelete: '¿Eliminar esta relación?',
  },
  outline: {
    title: 'Esquema',
    subtitle:
      'Organiza capítulos y escenas según tu esqueleto (Planteamiento, tres Desastres, Final).',
    newChapter: 'Nuevo capítulo',
    editChapter: 'Editar capítulo',
    chapterTitle: 'Título',
    chapterTitlePlaceholder: 'Título del capítulo',
    summary: 'Resumen (opcional)',
    summaryPlaceholder: 'Resumen corto. Usa /ai + Expandir con IA.',
    newScene: 'Nueva escena',
    editScene: 'Editar escena',
    chapter: 'Capítulo',
    selectChapter: 'Elegir capítulo',
    sceneTitle: 'Título',
    sceneTitlePlaceholder: 'Título de la escena',
    oneSentenceSummary: 'Resumen en una frase',
    oneSentencePlaceholder: 'Qué ocurre en esta escena. Usa /ai + Expandir con IA.',
    povCharacter: 'Personaje POV',
    notes: 'Notas',
    notesPlaceholder: 'Notas para escribir después. Usa /ai + Expandir con IA.',
    empty: 'Aún no hay capítulos. Añade un capítulo y luego escenas.',
    addFirstChapter: 'Añadir tu primer capítulo',
    untitledChapter: 'Capítulo sin título',
    untitledScene: 'Escena sin título',
    addScene: 'Añadir escena',
    chapterSummary: 'Resumen del capítulo',
    sceneOneSentence: 'Una frase de la escena',
    sceneNotes: 'Notas de la escena',
    confirmChapter: '¿Eliminar este capítulo y todas sus escenas?',
    confirmScene: '¿Eliminar esta escena?',
    writeScene: 'Escribir',
    sectionLabel: 'Sección del esqueleto',
    section: {
      setup: 'Planteamiento',
      disaster1: 'Desastre 1',
      disaster2: 'Desastre 2',
      disaster3: 'Desastre 3',
      ending: 'Final',
      ungrouped: 'Sin asignar',
      act1: 'Acto 1',
      act2a: 'Acto 2A',
      midpoint: 'Punto medio',
      act2b: 'Acto 2B',
      act3: 'Acto 3',
      you: 'Tú',
      need: 'Necesidad',
      go: 'Partir',
      search: 'Buscar',
      find: 'Encontrar',
      take: 'Pagar el precio',
      return: 'Retorno',
      change: 'Cambio',
      departure: 'Partida',
      road_of_trials: 'El camino de las pruebas',
      ordeal: 'Ordalia',
      ki: '起',
      sho: '承',
      ten: '转',
      ketsu: '合',
    },
    sectionTip: {
      setup:
        'Inicio: presenta el mundo, el protagonista y la situación antes de que todo se complique.',
      disaster1:
        'Primer desastre: rompe el statu quo y empuja al protagonista a una nueva situación.',
      disaster2:
        'Segundo desastre: sube las apuestas, profundiza el conflicto y empeora la situación.',
      disaster3: 'Tercer desastre / clímax: la mayor crisis, donde todo está en juego.',
      ending: 'Final: resolución y consecuencias después del clímax.',
      ungrouped:
        'Capítulos que aún no están vinculados a una sección del esqueleto. Asígnalos para mantener la estructura alineada.',
      act1: 'De la imagen de apertura al inicio del segundo acto: establece el tono, presenta al protagonista y lanza el incidente desencadenante.',
      act2a:
        'Diversión y juegos: la promesa de la premisa cumplida — el protagonista en el nuevo mundo, explorando antes de que lleguen las apuestas reales.',
      midpoint:
        'El giro central: una falsa victoria o falsa derrota que sube las apuestas y cambia la dirección.',
      act2b:
        'Los antagonistas se acercan hasta que todo está perdido: presión creciente, traiciones y el momento más oscuro antes del final.',
      act3: 'Inicio del tercer acto hasta la imagen final: el protagonista sintetiza lo aprendido y resuelve el conflicto.',
      you: 'El protagonista en su mundo cómodo y familiar — estasis antes del viaje.',
      need: 'Algo falta o no está bien — ¿qué necesita el protagonista, aunque no lo sepa?',
      go: 'El protagonista cruza hacia una situación o mundo desconocido.',
      search: 'Adaptarse, intentar, fallar, y hacer aliados y enemigos en el nuevo mundo.',
      find: 'El protagonista encuentra lo que buscaba.',
      take: 'Pero conseguirlo tiene un precio — un momento de sacrificio o consecuencia.',
      change: 'La transformación está completa — ya no es quien era al principio.',
      departure:
        'Establece el mundo ordinario del héroe, presenta la llamada a la aventura y muéstralo cruzando el umbral hacia lo desconocido.',
      road_of_trials:
        'El héroe afronta pruebas, hace aliados y enemigos, y madura a lo largo del viaje.',
      ordeal:
        'La crisis suprema — el héroe afronta su mayor miedo y vive una muerte y renacimiento simbólicos.',
      return: 'El héroe regresa a casa, transformado, portando lo que ha ganado.',
      ki: 'Presenta tus personajes, el mundo y la situación inicial. No se necesita conflicto — solo establece el escenario.',
      sho: 'Desarrolla y profundiza lo presentado. Construye atmósfera, relaciones y textura.',
      ten: 'Introduce un elemento inesperado — una revelación, un cambio de perspectiva o una yuxtaposición. Esto reencuadra lo anterior.',
      ketsu: 'Reúne todos los hilos. El giro de Ten encuentra aquí su significado.',
    },
    spineLabel: 'Nota del esqueleto para esta sección',
    spineNotFilled:
      'Esta parte del esqueleto aún no está rellenada en la pestaña Historia. Puedes seguir esquematizando, pero conviene completarla para tener mejor guía.',
    noChaptersInSection:
      'Todavía no hay capítulos en esta sección. Añade uno o deja que la IA proponga sugerencias.',
    aiDraftAll: 'Esquema completo con IA desde el esqueleto',
    aiDraftSection: 'Esquema de esta sección con IA',
    aiDraftTitle: 'Borrador de esquema con IA a partir del esqueleto',
    aiDraftSectionTitle: 'Borrador IA para {section}',
    aiDraftHint:
      'Revisa y ajusta estas sugerencias de capítulos y escenas. Al hacer clic en Aplicar, se crearán como capítulos y escenas reales en las secciones correspondientes.',
    aiDraftScenesLabel: 'Escenas de este capítulo',
    aiDraftEmptySection: 'No hay sugerencias para esta sección en el borrador actual.',
    aiDraftApply: 'Aplicar esquema',
    aiDraftError:
      'No se pudo generar el esquema con IA. Revisa tu clave en Ajustes → IA y vuelve a intentarlo.',
    aiDrafting: 'Esquematizando…',
    aiExtraContextChapterPrefix:
      'Este capítulo debe encajar en la sección «{section}» del esqueleto. Mantente coherente con esa parte:',
    aiExtraContextScenePrefix:
      'Esta escena debe encajar en la sección «{section}» del esqueleto. Mantente coherente con esa parte:',
    editInStory: 'Editar en Historia →',
    spineRefLabel: 'Eje de la historia',
    addFirstScene: 'Añadir primera escena',
    sceneWrittenAria: 'Escrita',
    sceneEmptyAria: 'Sin escribir',
  },
  write: {
    title: 'Escribir',
    subtitle: 'Elige una escena para escribir o editar. El texto se guarda automáticamente.',
    empty: 'Aún no hay escenas. Añade capítulos y escenas en el Esquema.',
    goToOutline: 'Ir al Esquema',
    generateContinue: 'Generar desde donde lo dejé',
    generateContinueHint:
      'La IA escribirá las escenas sin contenido a partir de "{name}". Las escenas ya escritas no se tocarán.',
    generateAll: 'Regenerar todo',
    generateAllDone: 'Todas las escenas ya están escritas.',
    generateAllDoneHint:
      'Cada escena tiene contenido. Puedes regenerar todo, pero se sobrescribirá la prosa existente.',
    generateConfirm:
      '¿Generar {count} escena(s) a partir de "{name}"? Las escenas anteriores ya escritas no se modificarán.',
    generateAllConfirm:
      '¿Regenerar las {count} escena(s)? Esto sobrescribirá toda la prosa existente.',
    generating: 'Generando…',
    generatingProgress: 'Escena {current} de {total}: {name}',
    generateDone: '{generated} escena(s) generada(s).',
    generatePartial:
      '{generated} generada(s), {errors} fallida(s). Comprueba tu clave API en Ajustes.',
    totalWords: 'palabras totales',
    scenesWritten: 'escenas escritas',
    scenesTotal: 'escenas totales',
    words: 'palabras',
    notStarted: 'no iniciada',
  },
  writeQuick: {
    capture: 'Capturar selección',
    saveAsIdea: 'Guardar como idea',
    addToCharacter: 'Añadir a personaje',
    addToOutline: 'Añadir al esquema',
    optional: 'opcional',
    characterAction: 'Acción',
    newCharacter: 'Nuevo personaje',
    existingCharacter: 'Personaje existente',
    pickCharacter: 'Elegir personaje',
    outlineAction: 'Acción',
    newScene: 'Nueva escena (este capítulo)',
    newChapter: 'Nuevo capítulo',
    noteForScene: 'Nota para esta escena',
    ideaSaved: 'Guardado como idea',
    characterSaved: 'Personaje añadido',
    linkedToCharacter: 'Vinculado a {name}',
    sceneSaved: 'Escena añadida',
    chapterSaved: 'Capítulo añadido',
    noteSaved: 'Nota añadida a la escena',
  },
  scene: {
    loading: 'Cargando…',
    notFound: 'Escena no encontrada.',
    backToWrite: 'Volver a Escribir',
    prose: 'Texto',
    prosePlaceholder: 'Escribe tu escena aquí…',
    save: 'Guardar',
    wordCountSingle: '1 palabra',
    wordCountPlural: 'palabras',
    focusMode: 'Modo enfoque',
    enterFocus: 'Modo enfoque',
    exitFocus: 'Salir (Esc)',
    generateThisScene: 'Generar esta escena',
    generatingScene: 'Generando…',
    generateError: 'No se pudo generar. Añade la clave API en Ajustes.',
    checkConsistency: 'Verificar consistencia',
    checkingConsistency: 'Verificando…',
    consistencyResult: 'Consistencia',
    noContradictions: 'Sin contradicciones.',
  },
  settings: {
    title: 'Ajustes',
    subtitle: 'Apariencia, idioma, IA y copia de seguridad.',
    appearance: 'Apariencia',
    darkMode: 'Tema',
    themeLight: 'Claro',
    themeDark: 'Oscuro',
    themeSystem: 'Sistema',
    whatsNew: 'Novedades',
    changelogTitle: 'Novedades en InkFlow',
    changelogClose: 'Cerrar',
    shortcuts: {
      title: 'Atajos de teclado',
      saveScene: 'Guardar escena (editor de escenas)',
      exitFocus: 'Salir del modo foco',
      globalSearch: 'Búsqueda global',
    },
  },
  export: {
    title: 'Exportar',
    subtitle:
      'Respalda tu proyecto como JSON. Luego puedes importarlo o usarlo en otro dispositivo.',
    language: 'Idioma',
    languageSubtitle: 'Idioma de la app y de la escritura.',
    ai: 'IA (Expandir con /ai)',
    aiHint:
      'Escribe una idea corta en cualquier tarjeta, luego /ai y "✨ Expandir con IA" para obtener un párrafo más completo usando tus Ideas e Historia. Las claves solo se guardan en este navegador.',
    provider: 'Proveedor',
    apiKey: 'Clave API',
    apiKeyHint: 'Obtén tu clave API en la plataforma del proveedor:',
    apiKeyGetKey: 'Obtener clave API',
    apiKeyPrivacyNote:
      'Las claves API se guardan solo en el localStorage de tu navegador. Nunca se envían a nuestros servidores.',
    apiKeyStoredLocally: 'Las claves API se guardan solo en este dispositivo.',
    show: 'Mostrar',
    hide: 'Ocultar',
    saveKey: 'Guardar clave',
    testApi: 'Probar API',
    testing: 'Probando…',
    exportTitle: 'Exportar',
    backupNudge: 'Llevas tiempo sin descargar un respaldo JSON. Considera guardar uno ahora.',
    exportErrorGeneric: 'No se pudo exportar. Vuelve a intentarlo o copia el texto en otro sitio.',
    exportFormat: 'Formato',
    formatJson: 'JSON — respaldo completo',
    formatJsonHint:
      'Guarda todo: estructura narrativa, personajes, ideas, capítulos y escenas. Úsalo para restaurar el proyecto en otro dispositivo.',
    formatStoryHint: 'Solo prosa de la historia — títulos de capítulos y contenido de escenas.',
    formatMarkdown: 'Markdown (.md)',
    formatTxt: 'Texto plano (.txt)',
    formatPdf: 'PDF (diálogo de impresión)',
    formatEpub: 'EPUB (.epub)',
    formatDocx: 'Word (.docx)',
    download: 'Descargar',
    pdfNote: 'Se abrirá un diálogo de impresión — elige "Guardar como PDF" en tu navegador.',
    importTitle: 'Importar',
    importBackup: 'Restaurar desde respaldo',
    importHint: 'Carga un respaldo JSON. Esto reemplazará tu proyecto actual — úsalo con cuidado.',
    importConfirmTitle: '¿Reemplazar el proyecto?',
    importConfirm: 'Esto reemplazará tu proyecto actual con el respaldo. No se puede deshacer.',
    importConfirmButton: 'Sí, importar',
    importDone: '¡Importación lista! Recargando…',
    importErrorGeneric: 'Importación fallida. Tu proyecto actual no se ha modificado.',
    testSuccess: '✓ La clave API funciona. Ya puedes usar Expandir con IA.',
    testError: 'Introduce una clave API y haz clic en Probar API.',
    keySaved: 'Clave API guardada.',
    qualityBias: 'Calidad de IA',
    qualityBiasFaster: 'Más rápido',
    qualityBiasBest: 'Mejor calidad',
    qualityBiasHint:
      '"Más rápido" usa modelos ligeros para menor coste y mayor velocidad. "Mejor calidad" mejora la escritura de escenas, esquemas y expansión de campos usando el modelo avanzado.',
  },
  storySwitcher: {
    current: 'Actual',
  },
  search: {
    placeholder: 'Buscar ideas, personajes, escenas…',
    typeToSearch: 'Escribe 2+ caracteres para buscar',
    noResults: 'Sin resultados',
    typeIdea: 'Idea',
    typeCharacter: 'Personaje',
    typeChapter: 'Capítulo',
    typeScene: 'Escena',
  },
  ai: {
    expandWithAi: '✨ Expandir con IA',
    titleExpand: 'Expandir esto con IA usando Ideas e Historia',
    titleSetup: 'Configura tu clave API primero',
    setupTitle: 'Configura la IA para usar Expandir con IA',
    setupText:
      'Añade tu clave API en Exportar → IA. Puedes usar Google Gemini (gratis) u OpenAI. La clave solo se guarda en este navegador.',
    goToSetup: 'Ir a Configuración',
    close: 'Cerrar',
    checkKey: 'Revisar clave o Probar API en Exportar → IA',
    previewTitle: 'Sugerencia de la IA',
    previewHint:
      'Puedes editar el texto y luego Aplicar, probar de nuevo otra idea, o Quedarte con el tuyo.',
    previewPlaceholder: 'Texto generado por la IA…',
    apply: 'Aplicar',
    tryAgain: 'Probar de nuevo',
    keepMine: 'Quedarme con el mío',
    generating: 'Generando…',
  },
  pip: {
    storyCompanion: 'Compañero de escritura',
    readingStory: 'Leyendo tu historia…',
    thinking: 'Pip está pensando…',
    inputPlaceholder: 'Habla con Pip…',
    noApiKey: 'Configura tu clave API en Ajustes para chatear con Pip 🦦',
    goToSettings: 'Ir a Ajustes',
    clearChat: 'Borrar chat',
    clearChatConfirm: '¿Borrar el historial del chat para esta historia?',
    welcomeDefault:
      '¡Hola! Soy Pip 🦦 Cuéntame la historia que quieres escribir — ¡o salúdame y encontraremos una idea juntos!',
    welcomeEmpty: '¡Hola! Soy Pip 🦦 {question}',
    welcomeProgress: '¡Buen progreso! Sigamos — {question}',
    welcomeNeedCharacters:
      '¡La estructura de tu historia está completa! Ahora démosles vida a tus personajes. ¿Quién es tu protagonista?',
    welcomeNeedChapters: '¡Buena base! ¿Quieres empezar a planificar tus capítulos?',
    welcomeSceneWithProse:
      "Trabajando en '{title}' — {n} palabras escritas. ¿En qué puedo ayudarte?",
    welcomeSceneNoProse: "¿Listo para trabajar en '{title}'? Puedo escribirla o pensamos juntos.",
    welcomeBack:
      '¡Bienvenido de vuelta! Has escrito {n} escenas en {chapters} capítulos. ¿Quieres continuar o centrarte en algo específico?',
    welcomeHasOutline:
      'Tu esquema está listo: {chapters} capítulos, {n} escenas. ¿Listo para empezar a escribir?',
    coachQuestion: {
      snowflake: {
        oneSentence: 'Cuéntame tu idea de historia — incluso una frase es suficiente para empezar.',
        setup: '¿Cómo comienza la historia? ¿Cómo era el mundo antes de que todo cambiara?',
        disaster1:
          '¿Cuál es el primer gran punto de inflexión — el momento en que las cosas salen mal?',
        disaster2:
          '¿Cómo escalan las apuestas? ¿Qué hace las cosas aún más difíciles para tu protagonista?',
        disaster3: '¿Cuál es el momento más oscuro — cuando toda esperanza parece perdida?',
        ending: '¿Cómo se resuelve? ¿Qué ha aprendido o en qué se ha convertido tu protagonista?',
      },
      hero_journey: {
        premise: 'Háblame de tu héroe y su mundo antes de que comience la aventura.',
        call: '¿Qué evento o encuentro aleja a tu héroe de su vida ordinaria?',
        trials: '¿Qué desafíos, aliados y enemigos encontrará tu héroe en el camino?',
        ordeal:
          '¿Cuál es el momento más oscuro — la crisis que tu héroe debe superar para renacer?',
        elixir: '¿Qué trae tu héroe de vuelta y cómo lo ha cambiado el viaje?',
      },
      kishotenketsu: {
        ki: 'Presenta tu historia — ¿quiénes son los personajes y cuál es la situación inicial?',
        sho: '¿Cómo se desarrolla la situación? ¿Qué detalles y profundidad puedes añadir?',
        ten: '¿Cuál es el giro inesperado o cambio de perspectiva? Recuerda — no tiene que ser un conflicto.',
        ketsu: '¿Cómo se reúnen todos los hilos al final?',
      },
      save_the_cat: {
        logline: 'Dame tu presentación en una frase — ¿quién es el héroe, qué quiere y qué se lo impide?',
        catalyst: '¿Cuál es el evento desencadenante que obliga a tu héroe a dejar su mundo ordinario y entrar en el acto dos?',
        debate: '¿Qué hace tu héroe con su nueva situación? ¿Qué tiene de divertido, emocionante o desafiante este nuevo mundo?',
        midpoint: '¿Qué ocurre en el punto medio que eleva las apuestas y empuja a tu héroe de lo reactivo a lo proactivo?',
        allIsLost: '¿Cuál es el momento más oscuro — cuando todo por lo que tu héroe ha trabajado parece destruido?',
        finale: '¿Cómo saca tu héroe fuerzas de flaqueza y logra la victoria? ¿Qué ha aprendido realmente o en qué se ha convertido?',
      },
      story_circle: {
        you: '¿Quién es tu protagonista y cómo es su mundo cómodo y estable al principio?',
        need: '¿Qué necesita tu protagonista, consciente o inconscientemente, que su vida actual no puede proporcionarle?',
        go: '¿Qué empuja o atrae a tu protagonista fuera de su zona de confort y hacia lo desconocido?',
        search: '¿Qué hace tu protagonista para intentar conseguir lo que necesita en esta situación desconocida?',
        find: '¿Qué encuentra o logra tu protagonista — aquello que estaba buscando?',
        take: '¿Qué precio debe pagar tu protagonista para conservar lo que encontró? ¿Qué le es arrebatado?',
        return: '¿Cómo regresa tu protagonista a su mundo familiar, transformado por la experiencia?',
        change: '¿En qué es fundamentalmente diferente tu protagonista al final? ¿En qué se ha convertido realmente?',
      },
    },
  },
  feedback: {
    button: 'Enviar comentarios',
    title: 'Enviar comentarios',
    typeLabel: 'Tipo',
    typeBug: 'Error',
    typeSuggestion: 'Sugerencia',
    typePraise: 'Cumplido',
    typeOther: 'General',
    bodyLabel: 'Tu comentario',
    bodyPlaceholder: 'Cuéntanos qué piensas…',
    submit: 'Enviar',
    required: 'Por favor, escribe algún comentario.',
    thankYou: '¡Gracias!',
    thankYouMsg:
      'Tu comentario ha sido copiado al portapapeles. Puedes pegarlo en un correo o en un issue de GitHub.',
    done: 'Listo',
  },
  entitySuggestion: {
    found: 'Pip encontró {count} nuevos elementos',
    foundOne: 'Pip encontró 1 nuevo elemento',
    review: 'Ver',
    collapse: 'Ocultar',
    saveSelected: 'Guardar en Ideas',
    skip: 'Omitir todo',
    selectAll: 'Seleccionar todo',
    saved: '{count} ideas guardadas',
    savedOne: '1 idea guardada',
    typeLabels: {
      character: 'Personaje',
      location: 'Lugar',
      world: 'Mundo',
      magic_system: 'Magia',
      symbol: 'Símbolo',
      mystery: 'Misterio',
      other: 'Otro',
    },
  },
  consistencyWarning: {
    found: 'Se encontraron {count} posibles contradicciones',
    foundOne: 'Se encontró 1 posible contradicción',
    review: 'Ver',
    collapse: 'Ocultar',
    dismiss: 'Ignorar',
    note: 'Revisa manualmente — no se ha modificado nada.',
  },
  novelImport: {
    buttonLabel: 'Importar desde texto de novela ✨',
    modalTitle: 'Importar novela',
    step1Title: 'Sube el archivo de tu novela',
    step2Title: 'Revisar estructura detectada',
    step3Title: 'Confirmar importación',
    uploadLabel: 'Elegir archivo (.txt o .md)',
    fileLoaded: 'Archivo cargado — listo para analizar',
    uploadHint: 'Admite archivos .txt y .md',
    uploadHintWithEpub: 'Admite archivos .txt, .md y .epub',
    epubInvalid: 'Este archivo EPUB parece inválido o está protegido con DRM.',
    epubEmpty: 'No se encontró texto legible en este archivo EPUB.',
    charsLoaded: 'caracteres cargados',
    analyzeBtn: 'Analizar',
    analyzing: 'Analizando…',
    backBtn: 'Volver',
    importBtn: 'Importar al proyecto',
    importing: 'Importando…',
    cancelBtn: 'Cancelar',
    chaptersFound: 'Se detectaron {count} capítulos',
    oneChapterFound: 'Se detectó 1 capítulo',
    scenesFound: '{count} escenas',
    charactersFound: 'Se detectaron {count} personajes',
    noCharacters: 'No se detectaron personajes',
    detectedTemplate: 'Estructura detectada: {name}',
    templateConfidence: 'Confianza: {pct}%',
    lowConfidenceWarning: 'La confianza en la estructura es baja — revisa antes de importar.',
    spinePreviewTitle: 'Vista previa de la columna vertebral',
    emptySpine: '(No detectado)',
    confirmTitle: 'Esto reemplazará el esquema, los personajes y la estructura actuales.',
    confirmHint: 'Las escenas y personajes existentes se eliminarán. Esta acción no se puede deshacer.',
    successToast: '¡Novela importada correctamente!',
    errorNoText: 'Por favor, sube un archivo primero.',
    errorTooShort: 'El texto es demasiado corto para analizar (mínimo 200 caracteres).',
    errorGeneric: 'Error al importar. Por favor, inténtalo de nuevo.',
    storyTitlePlaceholder: 'Título de la novela (opcional)',
  },
}
