  ...
  <div class="page">
    ...
    <div class="outline-sections">
      <section
        ...
        :class="{
          'outline-section--ungrouped': beat === 'ungrouped',
          'outline-section--empty': !chaptersForBeat(beat).length,
        }"
      >
        <!-- Section header -->
          ...
          <div class="outline-section-header-right">
            <div v-if="beat !== 'ungrouped'" class="outline-section-actions" @click.stop>
              <button
                ...
                @click="openNewChapterPanel(beat)"
              >
                + {{ t('outline.newChapter') }}
              </button>
              <button
                ...
                @click="draftForBeat(beat)"
                :disabled="drafting"
              >
                {{ t('outline.aiDraftSection') }}
              </button>
            </div>
            <span class="outline-section-chevron" :class="{ collapsed: collapsedBeats[beat] }"
              >▾</span
            >
          </div>
        </button>
          ...
          <div class="outline-chapter-list" :data-sortable-chapter-list="beat">
            ...
            >
              ...
              <div class="outline-chapter-row">
                ...
                <button type="button" class="outline-chapter-toggle" @click="toggleChapter(ch.id)">
                  <div class="outline-chapter-info">
                    <span class="outline-chapter-title">{{
                  ...
                  <div class="outline-chapter-progress">
                    ...
                    ></span>
                    <span v-if="scenesByChapter(ch.id).length > 8" class="scene-dots-more"
                      >+{{ scenesByChapter(ch.id).length - 8 }}</span
                    >
                  </div>
                  <span
                    class="outline-chapter-chevron"
                    :class="{ collapsed: collapsedChapters[ch.id] }"
                    >▾</span
                  >
                </button>
                ...
                <div class="outline-chapter-actions">
                  <button
                    ...
                    :title="t('ideas.edit')"
                    @click.stop="openEditChapterPanel(ch)"
                  >
                    ✏️
                  </button>
                  <button
                    type="button"
                    class="btn btn-ghost btn-sm btn-icon"
                    :title="t('outline.addScene')"
                    @click.stop="openAddScenePanel(ch.id, beat)"
                  >
                    <svg
                      width="14"
                      height="14"
              ...
              >
                ...
                >
                  <span class="scene-drag-handle" aria-label="Reorder scene" title="Drag to reorder"
                    >≡</span
                  >
                  <span
                    ...
                    :title="
                      isSceneWritten(scene)
                        ? t('outline.sceneWrittenAria')
                        : t('outline.sceneEmptyAria')
                    "
                  ></span>
                ...
                <div v-if="!scenesByChapter(ch.id).length" class="outline-scenes-empty">
                  <button
                    type="button"
                    class="btn btn-ghost btn-sm"
                    @click="openAddScenePanel(ch.id, beat)"
                  >
...
function beatColor(b) {
  return getBeatColor(story.value, b)
}
...
function toggleBeat(b) {
  ...
  try {
    localStorage.setItem(COLLAPSE_KEY, JSON.stringify({ ...collapsedBeats }))
  } catch {}
}
...
function sceneDotsForChapter(chapterId) {
  return scenesByChapter(chapterId)
    .slice(0, 8)
    .map((s) => isSceneWritten(s))
}
...
// --- After-change hook: reload data, notify stores, re-init sortables ---
...
function initSortables() {
  ...
  document.querySelectorAll('[data-sortable-chapter-list]').forEach((el) => {
    ...
    const sortable = Sortable.create(el, {
      ...
      animation: 150,
      onEnd() {
        const ids = [...el.querySelectorAll('[id^="chapter-"]')].map((node) =>
          node.id.replace('chapter-', '')
        )
  ...
  document.querySelectorAll('[data-sortable-scene-list]').forEach((el) => {
    ...
    const sortable = Sortable.create(el, {
      ...
      animation: 150,
      onEnd() {
        const ids = [...el.querySelectorAll('[id^="scene-"]')].map((node) =>
          node.id.replace('scene-', '')
        )
        if (ids.length === 0) return
...
onMounted(async () => {
  ...
  await nextTick()
  initSortables()
  // Reload when story or characters change
  watch(() => storyStore.activeStoryId, async () => {
    await loadAll()
    await nextTick()
    initSortables()
  })
  watch(() => storyStore.activeStory, async () => { await loadAll() }, { deep: true })
  watch(() => charactersStore.characters, async () => { await loadAll() }, { deep: true })
})
onUnmounted(() => {
...
.outline-spine-ref-text {
  margin: 0;
  font-size: 0.9375rem;
  color: var(--text);
  line-height: 1.5;
}
.outline-spine-ref-link {
  font-size: 0.8125rem;
  color: var(--accent);
...
.outline-section-titleblock {
  flex: 1;
  min-width: 0;
}
.outline-section-title-row {
  display: flex;
  align-items: center;
  gap: var(--space-2);
...
.outline-section-chevron.collapsed {
...
.outline-chapter-chevron {
  font-size: 0.875rem;
  color: var(--text-muted);
  transition: transform 0.2s ease;
  flex-shrink: 0;
}
.outline-chapter-chevron.collapsed {
  transform: rotate(-90deg);
}
...
.scene-status-indicator--written {
...
.outline-scene-actions .btn-sm {
  white-space: nowrap;
}

.outline-scenes-empty {
